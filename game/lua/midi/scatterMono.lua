...
function ScatterMidi:onUpdate(dt)
	...
	if self.device and self.t > self.next then
		...
		local newNote = noten < 1 or lovr.math.random() > 0.5
		if not newNote and noten == 1 then
			midi.note(self.device, false, self.notes[1])
			self.notes[1] = nil
			noten = 0
			newNote = true
		end
		if newNote then
			...
			if collide then return
			else
				if noten > 1 then
					midi.note(self.device, false, self.notes[1])
					for i=1,noten do self.notes[i] = self.notes[i+1] end
				end 
				table.insert(self.notes, note)
			end
			midi.note(self.device, true, note, lovr.math.random(0, 0x7F))
		else
