...
static void onCapture(ma_device* device, void* outputUntyped, const void* inputUntyped, uint32_t frames) {
  ...
  while(frames > 0) {
    ...
    input += availableFrames * CAPTURE_CHANNELS; // FIXME: Bytesperframe?
  }
}
...
#ifdef LOVR_DEBUG_AUDIOTAP
  lovrFilesystemWrite("lovrDebugAudio.raw", NULL, 0, false);
...
// FIXME: Lock is needed, but lock can't be held within onPlayback
